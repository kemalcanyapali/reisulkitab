...
import hotkey
import ipc
from tests.support import AppTest, fake_urlopen
...
class Printing(unittest.TestCase):
    ...
    def test_a_failure_as_json_stays_on_stdout(self):
        ...
        self.assertEqual(err.getvalue(), "")
    def test_windows_standard_streams_are_utf8(self):
        ...
        with mock.patch.object(cli.os, "name", "nt"), \
            ...
            cli._utf8_stdio()
        stdin.reconfigure.assert_called_once_with(
...
class Masking(unittest.TestCase):
    ...
    def test_every_stored_secret_is_masked(self):
        self.assertEqual(set(cli.SECRET_KEYS), set(cfg._SECRET_SETTINGS))
        for key in cfg._SECRET_SETTINGS:
...
class Providers(AppTest):
    ...
    def test_key_uses_the_configured_compatible_endpoint(self):
        base = "https://generativelanguage.googleapis.com/v1beta/openai"
        self.write_config({
            "openrouter_api_key": "gemini-key",
            "openrouter_base_url": base,
        })
        with fake_urlopen({"data": [{"id": "gemini-3.6-flash"}]}) as calls:
            code, _, _ = self.run_cmd(cli.cmd_test_key, which="openrouter")
        self.assertEqual(code, 0)
        self.assertEqual(calls[0].full_url, f"{base}/models")


class Doctor(AppTest):
    ...
    def test_cleanup_on_a_cli_is_a_question_about_the_program(self):
        reply = self.run_doctor(cleanup_provider="claude",
                                cleanup_claude_model="haiku")
        self.assertEqual(reply["cleanup"]["provider"], "claude")
        self.assertEqual(reply["cleanup"]["model"], "haiku")
        self.assertIn("claude", reply["programs"])
        self.assertIn("claude, cleaning up on haiku",
                      self.run_doctor(as_json=False, cleanup_provider="claude",
                                      cleanup_claude_model="haiku"))
    def test_local_transcription_checks_the_runtime_and_prints_on_windows(self):
        with mock.patch.object(cli.ggml, "program_path", return_value="whisper"), \
                mock.patch.object(cli.ggml, "have_model", return_value=True):
            reply = self.run_doctor(as_json=False, transcribe_provider="local")
        self.assertIn("v local whisper.cpp", reply)
        reply.encode("cp1252")



class Finding(AppTest):
...
class Launching(unittest.TestCase):
    def test_source_launch_includes_the_script(self):
        with mock.patch.object(cli.os, "execv") as execv:
            cli.launch_gui("toggle")
        execv.assert_called_once_with(
            cli.sys.executable,
            [cli.sys.executable, ipc.script_path(), "toggle", "--gui"],
        )

    def test_frozen_windows_launch_spawns_the_requested_verb(self):
...
class WithoutAnInstance(AppTest):
    ...
    def test_malformed_config_diagnostic_stays_off_json_stdout(self):
        cfg.CONFIG_DIR.mkdir(parents=True, exist_ok=True)
        cfg.CONFIG_FILE.write_text("{not json", encoding="utf-8")
        with captured() as (out, err):
            code = cli.run(["--json", "config", "get", "cleanup_model"])
        self.assertEqual(code, 0)
        self.assertEqual(json.loads(out.getvalue())["value"],
                         cfg.DEFAULTS["cleanup_model"])
        self.assertIn("could not read settings", err.getvalue())


class Replies(AppTest):
