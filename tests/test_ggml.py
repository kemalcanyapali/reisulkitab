...
import ggml
import hub
from tests.support import (AppTest, fake_urlopen, http_error, json_body,
...
class Local(AppTest):
    ...
    def test_process_snapshot_handles_are_pointer_sized(self):
        self.assertTrue(ggml._process_descends_from(os.getpid(), os.getpid()))
...
class Servers(Local):
    ...
    def test_windows_kills_server_when_its_owner_job_closes(self):
        server = self.server()
        server.serve()
        proc = server._proc
        self.assertTrue(getattr(proc, "_reisulkuttab_job", None))
    ...
    @linux_only
    def test_a_server_a_killed_reisulkuttab_left_behind_is_swept_up(self):
        server = self.server()
        server.serve()
        # What a SIGKILL of Reisülküttab leaves: the child still running, the pid file
