...
import i18n
import paste
from tests.support import AppTest


class Loading(AppTest):
...
class Saving(AppTest):
    ...
    def test_concurrent_instances_merge_their_changes(self):
        first = cfg.Config()
        second = cfg.Config()
        first["cleanup_provider"] = "claude"
        second["meeting_provider"] = "local"
        first.save()
        second.save()
        saved = cfg.Config()
        self.assertEqual(saved["cleanup_provider"], "claude")
        self.assertEqual(saved["meeting_provider"], "local")

    @unittest.skipIf(os.name == "nt", "Windows protects AppData with ACLs, not POSIX modes")
    ...
    def test_only_the_lock_and_final_file_remain(self):
        cfg.Config().save()
        self.assertEqual(
            sorted(p.name for p in cfg.CONFIG_DIR.iterdir()),
    ...
    def test_windows_encrypts_saved_api_keys_for_this_user(self):
        conf = cfg.Config()
        conf["openai_api_key"] = "sk-local-secret"
        conf.save()
        raw = json.loads(cfg.CONFIG_FILE.read_text(encoding="utf-8"))
        self.assertNotIn("sk-local-secret", cfg.CONFIG_FILE.read_text(encoding="utf-8"))
        self.assertTrue(raw["openai_api_key"].startswith("dpapi:"))
        self.assertEqual(cfg.Config().openai_key(), "sk-local-secret")
...
class Keys(AppTest):
    ...
    def test_the_environment_is_the_fallback(self):
        with mock.patch.object(cfg, "_ENV_KEYS", {"OPENAI_API_KEY"}), \
    ...
    def test_a_stored_key_beats_the_environment(self):
        with mock.patch.object(cfg, "_ENV_KEYS", {"OPENROUTER_API_KEY"}), \
    ...
    def test_every_provider_falls_back_to_the_variable_of_its_own_name(self):
        with mock.patch.object(cfg, "_ENV_KEYS", {"GROQ_API_KEY"}), \
                mock.patch.dict(os.environ, {"GROQ_API_KEY": "gsk-env"}):
            self.assertEqual(cfg.Config().groq_key(), "gsk-env")


class TranscribeTarget(AppTest):
...
class DotEnv(AppTest):
    def test_credentials_and_provider_choices_load_from_config_directory(self):
        cfg.CONFIG_DIR.mkdir(parents=True)
        (cfg.CONFIG_DIR / ".env").write_text(
            "OPENROUTER_API_KEY=secret\n"
            "REISULKUTTAB_CLEANUP_PROVIDER=agy\n"
            "REISULKUTTAB_MEETING_PROVIDER=openrouter\n",
            encoding="utf-8",
        )
        with mock.patch.object(
                cfg, "_ENV_KEYS",
                {"OPENROUTER_API_KEY", "REISULKUTTAB_CLEANUP_PROVIDER",
                 "REISULKUTTAB_MEETING_PROVIDER"}), \
                mock.patch.object(
                    cfg, "_env_paths",
                    lambda: (cfg.CONFIG_DIR / ".env",)), \
                mock.patch.dict(os.environ, {}, clear=True):
            conf = cfg.Config()
            self.assertEqual(conf.openrouter_key(), "secret")
            self.assertEqual(conf["cleanup_provider"], "agy")
            self.assertEqual(conf["meeting_provider"], "openrouter")
...
class Defaults(unittest.TestCase):
    ...
    def test_windows_defaults_are_offline_or_subscription_backed(self):
        self.assertEqual(cfg.DEFAULTS["cleanup_provider"], "local")
        self.assertEqual(cfg.DEFAULTS["cleanup_model"], "google/gemini-3.5-flash-lite")
        self.assertEqual(cfg.DEFAULTS["meeting_provider"], "agy")
        self.assertEqual(cfg.DEFAULTS["meeting_model"], "google/gemini-3.5-flash")
        self.assertTrue(cfg.DEFAULTS["meeting_keep_audio"])
        self.assertIn("Exactly five concise bullet points", cfg.MEETING_PROMPT_EN)
...
class Directories(unittest.TestCase):
    ...
    def test_linux_keeps_them_apart_and_follows_xdg(self):
        ...
        self.assertEqual(str(config_dir), "/c/reisulkuttab")
        self.assertEqual(str(data_dir), "/d/reisulkuttab")

    def test_linux_without_the_variables_set(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            config_dir, data_dir = cfg._directories("linux")
        self.assertTrue(str(config_dir).endswith("/.config/reisulkuttab"))
        self.assertTrue(str(data_dir).endswith("/.local/share/reisulkuttab"))
    ...
    def test_a_mac_keeps_both_in_application_support(self):
        ...
        self.assertTrue(str(config_dir).endswith("/Library/Application Support/Reisülküttab"))
    ...
    def test_windows_uses_roaming_settings_and_local_recordings(self):
        with mock.patch.dict(os.environ, {
                "APPDATA": r"C:\Users\me\AppData\Roaming",
                "LOCALAPPDATA": r"C:\Users\me\AppData\Local"}):
            config_dir, data_dir = cfg._directories("win32")
        self.assertEqual(str(config_dir), r"C:\Users\me\AppData\Roaming\reisulkuttab")
        self.assertEqual(str(data_dir), r"C:\Users\me\AppData\Local\reisulkuttab")

    def test_windows_import_paths_stay_inside_the_test_sandbox(self):
        if cfg.sys.platform != "win32":
            self.skipTest("Windows-only path assertion")
        self.assertTrue(
            cfg.CONFIG_DIR.is_relative_to(pathlib.Path(os.environ["APPDATA"])))
        self.assertTrue(
            cfg.DATA_DIR.is_relative_to(pathlib.Path(os.environ["LOCALAPPDATA"])))

    def test_the_predecessor_profile_is_moved_and_its_lock_is_removed(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            with mock.patch.dict(os.environ, environment), \
                    mock.patch.object(cfg.sys, "platform", "win32"):
                old_config, old_data = cfg._directories(
                    name="Dikte", slug="dikte")
                new_config, new_data = cfg._directories()
                old_config.mkdir(parents=True)
                old_data.mkdir(parents=True)
                (old_config / "config.json").write_text(
                    '{"cleanup_enabled": false}')
                (old_data / "history.jsonl").write_text(
                    '{"text": "kept"}\n')
                (old_data / "dikte.lock").write_text("stale")

                cfg._migrate_legacy_directories()

                self.assertFalse(old_config.exists())
                self.assertFalse(old_data.exists())
                self.assertTrue((new_config / "config.json").exists())
                self.assertTrue((new_data / "history.jsonl").exists())
                self.assertFalse((new_data / "dikte.lock").exists())

    def test_the_original_windows_xdg_profile_is_also_moved(self):
        with tempfile.TemporaryDirectory() as root:
            root = pathlib.Path(root)
            environment = {
                "APPDATA": str(root / "roaming"),
            ...
            with mock.patch.dict(os.environ, environment), \
                    mock.patch.object(cfg.sys, "platform", "win32"):
                old_config = pathlib.Path(environment["XDG_CONFIG_HOME"]) / "dikte"
                old_data = pathlib.Path(environment["XDG_DATA_HOME"]) / "dikte"
                new_config, new_data = cfg._directories()
                old_config.mkdir(parents=True)
                (old_data / "models").mkdir(parents=True)
                (old_config / "config.json").write_text("settings")
                (old_data / "models" / "model.bin").write_text("model")

                cfg._migrate_legacy_directories()

                self.assertEqual(
                    (new_config / "config.json").read_text(), "settings")
                self.assertEqual(
                    (new_data / "models" / "model.bin").read_text(), "model")
                self.assertFalse(old_config.exists())
                self.assertFalse(old_data.exists())

    def test_transitional_display_name_profile_moves_to_the_slug(self):
        with tempfile.TemporaryDirectory() as root:
            root = pathlib.Path(root)
            environment = {
                "APPDATA": str(root / "roaming"),
            ...
            with mock.patch.dict(os.environ, environment), \
                    mock.patch.object(cfg.sys, "platform", "win32"):
                old_config = pathlib.Path(environment["APPDATA"]) / "Reisülküttab"
                old_data = pathlib.Path(environment["LOCALAPPDATA"]) / "Reisülküttab"
                new_config, new_data = cfg._directories()
                old_config.mkdir(parents=True)
                old_data.mkdir(parents=True)
                (old_config / "config.json").write_text("settings")
                (old_data / "history.jsonl").write_text("history")
                ...
                self.assertEqual(
                    (new_config / "config.json").read_text(), "settings")
                self.assertEqual(
                    (new_data / "history.jsonl").read_text(), "history")
                self.assertFalse(old_config.exists())
                self.assertFalse(old_data.exists())

    def test_migration_merges_coexisting_profiles_without_overwriting(self):
        with tempfile.TemporaryDirectory() as root:
            root = pathlib.Path(root)
            environment = {
                "APPDATA": str(root / "roaming"),
            ...
            with mock.patch.dict(os.environ, environment), \
                ...
                old_config, old_data = cfg._directories(
                    name="Dikte", slug="dikte")
                new_config, new_data = cfg._directories()
                for directory in (old_config, old_data, new_config, new_data):
                    directory.mkdir(parents=True)
                (old_config / "config.json").write_text("old")
                (new_config / "config.json").write_text("new")
                ...
                self.assertEqual(
                    (new_config / "config.json").read_text(), "new")
                self.assertEqual(
                    (new_config / "config.json.predecessor").read_text(), "old")
                self.assertEqual(
                    (new_data / "models" / "model.bin").read_text(), "model")
                self.assertFalse(old_config.exists())
                self.assertFalse(old_data.exists())
    ...
    def test_only_the_cleanup_setting_asks_for_the_local_model(self):
        ...
        self.assertTrue(
            self.config(cleanup_enabled=True,
                        cleanup_provider="local").uses_local_llm())


class ReadyToRun(AppTest):
    def setUp(self):
        super().setUp()
