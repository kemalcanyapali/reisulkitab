...
import paste
from tests.support import AppTest, FakeCompleted, only_these_tools
...
class Standing:
    ...
    def setUp(self):
        super().setUp()
        self.patch_attr(sys, "platform", self.platform)
...
class ClipboardContract(Standing):
    ...
    def test_a_restore_failure_is_reported(self):
        """The caller can warn that the user's original clipboard was not restored."""
        with only_these_tools(self.here.clipboard), \
                mock.patch.object(subprocess, "run", side_effect=OSError("nope")), \
                self.assertRaises(paste.PasteError):
...
class Wayland(ClipboardContract, KeyProgramContract, AppTest):
...
class X11(ClipboardContract, KeyProgramContract, AppTest):
...
class MacOS(ClipboardContract, AppTest):
    platform = "darwin"
    here = paste.MACOS
...
class MacClipboardSnapshot(AppTest):
    def test_every_native_type_is_restored_and_the_files_are_removed(self):
        directory = tempfile.mkdtemp(prefix="reisulkuttab-test-clipboard-")
        ...
        self.assertEqual(run.call_args.kwargs["env"]["REISULKUTTAB_PASTEBOARD_DIR"],
                         directory)
...
class WindowsClipboard(AppTest):
    """The Win32/OLE clipboard APIs, without touching the real clipboard."""
    ...
    def setUp(self):
        ...
        windll = mock.Mock(
            user32=self.user32, kernel32=self.kernel32, ole32=self.ole32,
        )
        patcher = mock.patch.object(paste.ctypes, "windll", windll, create=True)
        patcher.start()
        self.addCleanup(patcher.stop)
        release_patcher = mock.patch.object(
            paste, "_release_windows_data_object", create=True,
        )
        self.release = release_patcher.start()
        self.addCleanup(release_patcher.stop)

        self.owner = 101
        self.user32.CreateWindowExW.return_value = self.owner
        self.user32.OpenClipboard.return_value = True
        self.user32.EmptyClipboard.return_value = True
        self.user32.CloseClipboard.return_value = True
        self.user32.DestroyWindow.return_value = True
        self.user32.EnumClipboardFormats.return_value = 0
        self.user32.GetClipboardSequenceNumber.return_value = 7

        self.allocations = {}
        self.freed = []

        def allocate(flags, size):
            allocation = ctypes.create_string_buffer(size)
            handle = ctypes.addressof(allocation)
            self.allocations[handle] = allocation
            return handle

        self.kernel32.GlobalAlloc.side_effect = allocate
        self.kernel32.GlobalLock.side_effect = lambda handle: handle
        self.kernel32.GlobalUnlock.return_value = True
        self.kernel32.GlobalFree.side_effect = self.freed.append
        self.user32.SetClipboardData.side_effect = (
            lambda clipboard_format, handle: handle
        )

        self.data_object = 0x1234
        self.ole32.OleInitialize.return_value = 0

        def capture(data_object):
            data_object._obj.value = self.data_object
            return 0

        self.ole32.OleGetClipboard.side_effect = capture
        self.ole32.OleSetClipboard.return_value = 0
        self.ole32.OleFlushClipboard.return_value = 0

    def test_copy_uses_a_real_owner_window(self):
        paste.copy("hello")

        owner = self.user32.OpenClipboard.call_args.args[0]
        self.assertEqual(owner, self.owner)
        self.assertIsNotNone(owner)
        self.user32.CreateWindowExW.assert_called_once()
        self.user32.DestroyWindow.assert_called_once_with(self.owner)
        self.assertEqual(self.freed, [])

    def test_copy_fails_if_set_clipboard_data_installs_nothing(self):
        self.user32.SetClipboardData.side_effect = None
        self.user32.SetClipboardData.return_value = None

        with self.assertRaises(paste.PasteError):
            paste.copy("hello")

        self.assertEqual(self.freed, list(self.allocations))
        self.user32.CloseClipboard.assert_called_once()
        self.user32.DestroyWindow.assert_called_once_with(self.owner)

    def test_snapshot_restores_every_native_format_then_releases_on_same_thread(self):
        """OleFlushClipboard makes delayed native formats independent of Reisülküttab."""
        snapshot = paste.read_clipboard()
        paste.copy("temporary")
        paste.copy_bytes(snapshot, expected_sequence=7)

        restored = self.ole32.OleSetClipboard.call_args.args[0]
        self.assertEqual(restored.value, self.data_object)
        self.ole32.OleFlushClipboard.assert_called_once_with()
        self.release.assert_called_once()
        self.ole32.OleUninitialize.assert_called_once()
        self.user32.EnumClipboardFormats.assert_not_called()
        self.user32.GetClipboardData.assert_not_called()

    def test_a_transient_ole_restore_failure_is_retried(self):
        snapshot = paste.read_clipboard()
        self.ole32.OleSetClipboard.side_effect = [-2147221040, 0]

        with mock.patch.object(paste.time, "sleep") as sleep:
            paste.copy_bytes(snapshot, expected_sequence=7)

        self.assertEqual(self.ole32.OleSetClipboard.call_count, 2)
        sleep.assert_called_once_with(0.01)
        self.ole32.OleFlushClipboard.assert_called_once_with()
        self.release.assert_called_once()

    def test_retry_does_not_overwrite_newer_clipboard_content(self):
        snapshot = paste.read_clipboard()
        self.user32.GetClipboardSequenceNumber.side_effect = [7, 8]
        self.ole32.OleSetClipboard.side_effect = [-2147221040, 0]

        with mock.patch.object(paste.time, "sleep") as sleep:
            paste.copy_bytes(snapshot, expected_sequence=7)

        self.ole32.OleSetClipboard.assert_called_once()
        sleep.assert_called_once_with(0.01)
        self.ole32.OleFlushClipboard.assert_not_called()
        self.release.assert_called_once()


    def test_newer_user_clipboard_content_is_not_overwritten(self):
        snapshot = paste.read_clipboard()
        self.user32.GetClipboardSequenceNumber.return_value = 8

        paste.copy_bytes(snapshot, expected_sequence=7)

        self.ole32.OleSetClipboard.assert_not_called()
        self.ole32.OleFlushClipboard.assert_not_called()
        self.release.assert_called_once()
        self.ole32.OleUninitialize.assert_called_once()

    def test_a_failed_ole_snapshot_balances_initialization(self):
        self.ole32.OleGetClipboard.side_effect = None
        self.ole32.OleGetClipboard.return_value = -1
