...
import meeting
from tests.support import AppTest, make_wav, silence, speech, stereo, tone
...
class Entry(unittest.TestCase):
    def test_the_stem_is_a_sortable_timestamp(self):
        ...
        self.assertRegex(base, r"^\d{4}-\d{2}-\d{2}-\d{4}(?:-\d+)?$")
...
class Pipeline(AppTest):
    """The chain, with the transcription and the two cleanup calls faked."""

    def setUp(self):
        super().setUp()
        self.conf = self.config(
            cleanup_provider="openrouter", meeting_provider="openrouter",
    ...
    def test_the_audio_goes_once_the_minutes_are_written(self):
        self.conf["meeting_keep_audio"] = False
        self.run_pipeline()
