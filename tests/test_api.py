...
class Headers(unittest.TestCase):
    ...
    def test_openrouter_is_told_who_is_calling_without_upstream_attribution(self):
        headers = api._headers("openrouter", "sk-or-test")
        self.assertNotIn("HTTP-Referer", headers)
        self.assertEqual(headers["X-Title"], "Reisülküttab")
...
class Transcribe(AppTest):
    ...
    def test_openrouter_is_attributed(self):
        ...
        self.assertEqual(calls[0].get_header("X-title"), "Reisülküttab")
...
class TranscribeSegments(AppTest):
    def setUp(self):
        super().setUp()
        self.wav = str(self.path("clip.wav"))
        os.makedirs(self.root, exist_ok=True)
...
class KeyStatus(AppTest):
    ...
    def test_a_custom_base_checks_its_model_endpoint(self):
        base = "https://generativelanguage.googleapis.com/v1beta/openai"
        with fake_urlopen({"data": [{"id": "gemini-3.6-flash"}]}) as calls:
            message = api.openrouter_key_status("gemini-key", base)
...
class TranscribeHere(AppTest):
    ...
    def test_the_hint_reaches_whisper_as_its_initial_prompt(self):
        with fake_urlopen({"text": "hi"}) as calls:
            api.transcribe(LOCAL, self.wav, prompt="Reisülküttab, Paraşüt")
        self.assertEqual(multipart_fields(calls[0])["prompt"], "Reisülküttab, Paraşüt")
