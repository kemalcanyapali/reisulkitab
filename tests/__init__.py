...
Every module resolves its paths at import time, so the native config and data
roots are redirected here: a test that forgets to ask for a throwaway directory
writes into a temporary one instead of the real user profile. The Qt platform is
pinned for the same reason, so a machine with no display and a CI runner behave
...
_SANDBOX = tempfile.mkdtemp(prefix="reisulkuttab-tests-")
