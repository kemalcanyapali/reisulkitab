...
class AppTest(unittest.TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.root = tempfile.mkdtemp(prefix="reisulkuttab-test-")
        self.addCleanup(shutil.rmtree, self.root, True)

        config_dir = self.path("config", "reisulkuttab")
        data_dir = self.path("data", "reisulkuttab")
        self.patch_paths(
        ...
        self.patch_attr(cfg, "_ENV_KEYS", frozenset())
        self.patch_attr(cfg, "_env_paths", lambda: ())
        ...
        ggml_data = self.path("local", "reisulkuttab")
        for name, path in {
                "DATA_DIR": ggml_data,
                "BIN_DIR": ggml_data / "bin",
                "MODELS_DIR": ggml_data / "models",
        }.items():
            self.patch_attr(ggml, name, path)
...
def fake_urlopen(*replies):
    ...
    try:
        with mock.patch("api._urlopen", side_effect=opener), \
