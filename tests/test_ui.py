...
import paste
import settings_ui
from tests.support import AppTest, only_these_tools
...
class Settings(AppTest):
    ...
    def test_installing_the_discard_key_writes_its_own_entry(self):
        ...
        self.assertEqual(command[-1], "cancel")
    ...
    def test_the_window_is_readable_in_turkish_too(self):
        ...
        window = self.window(cfg.Config())
        self.assertEqual(window.windowTitle(), "Reisülküttab Ayarları")
...
class WindowsSettings(Settings):
    ...
    def test_emptying_a_shortcut_turns_it_off_but_not_the_toggle(self):
        ...
        for box, _status, _missing in window._shortcut_rows.values():
            box.setCurrentText("")
        window._save()
        self.assertEqual(conf["shortcut"], "Ctrl+Alt+D")
        self.assertEqual(conf["cancel_shortcut"], "")
        self.assertEqual(conf["assistant_shortcut"], "")
        self.assertEqual(conf["meeting_shortcut"], "")

    def test_windows_shortcuts_are_active_without_install_buttons(self):
        window = self.window(cfg.Config())
        ...
        self.assertEqual(
            window._shortcut_rows["toggle"][1].text(),
            "Active while Reisülküttab is running.",
        )
...
class DictationWarnings(AppTest):
    def controller(self):
        controller = object.__new__(reisulkuttab.Reisulkuttab)
        controller.overlay = mock.Mock()
        controller.tray = mock.Mock()
        controller.conf = {"auto_paste": True}
        controller._set_state = mock.Mock()
        controller._settle = mock.Mock()
        return controller

    def test_a_clipboard_restore_warning_is_not_called_cleanup_failure(self):
        controller = self.controller()
        with mock.patch.object(
            reisulkuttab, "t", side_effect=lambda text, **values: text.format(**values)
        ):
            controller._on_finished(
                "raw", "Cleaned.", "", "Could not restore the Windows clipboard."
            )

        message = controller.overlay.show_warning.call_args.args[0]
        self.assertIn("clipboard", message)
        self.assertNotIn("cleanup failed", message)

    def test_cleanup_and_clipboard_warnings_both_reach_cli_and_tray(self):
        controller = self.controller()
        with mock.patch.object(
            reisulkuttab, "t", side_effect=lambda text, **values: text.format(**values)
        ):
            controller._on_finished(
                "raw", "raw", "cleanup unavailable", "clipboard is busy"
            )

        details = "cleanup unavailable\nclipboard is busy"
        self.assertEqual(controller.tray.showMessage.call_args.args[1], details)
        self.assertEqual(
            controller._settle.call_args.args[1]["warning"], details,
        )

    def test_cleanup_warning_does_not_claim_the_text_was_pasted(self):
        controller = self.controller()
        controller.conf["auto_paste"] = False
        with mock.patch.object(
            reisulkuttab, "t", side_effect=lambda text, **values: text.format(**values)
        ):
            controller._on_finished(
                "raw", "raw", "cleanup unavailable", ""
            )

        message = controller.overlay.show_warning.call_args.args[0]
        self.assertEqual(
            message, "Cleanup failed; raw text was used: cleanup unavailable",
        )
...
class LaunchAtLoginMigration(unittest.TestCase):
    ...
    def winreg(self, values, write_error=None):
        key = self.Key(values)

        def query(open_key, name):
            if name not in open_key:
                raise FileNotFoundError(name)
            return open_key[name], 1

        def write(open_key, name, _reserved, _kind, value):
            if write_error:
                raise write_error
            open_key[name] = value
        ...
        return types.SimpleNamespace(
            ...
            QueryValueEx=query,
            SetValueEx=write,
            DeleteValue=lambda open_key, name: open_key.pop(name),
            values=key,
        )

    def test_predecessor_startup_is_removed_only_after_replacement_exists(self):
        values = {"Dikte": "old command"}
        winreg = self.winreg(values)
        with mock.patch.object(reisulkuttab.sys, "platform", "win32"), \
                mock.patch.dict(sys.modules, {"winreg": winreg}), \
                mock.patch.object(
                    reisulkuttab, "_launch_command", return_value="new command"):
            reisulkuttab._migrate_launch_at_login()
            reisulkuttab._migrate_launch_at_login()

        self.assertEqual(winreg.values, {"Reisülküttab": "new command"})

    def test_existing_renamed_entry_is_refreshed_before_old_entry_removal(self):
        values = {
            "Dikte": "old command",
            "Reisülküttab": "transitional install command",
        }
        winreg = self.winreg(values)
        with mock.patch.object(reisulkuttab.sys, "platform", "win32"), \
                mock.patch.dict(sys.modules, {"winreg": winreg}), \
    ...
    def test_renamed_entry_is_refreshed_after_install_directory_changes(self):
        values = {"Reisülküttab": "transitional install command"}
        winreg = self.winreg(values)
        with mock.patch.object(reisulkuttab.sys, "platform", "win32"), \
                mock.patch.dict(sys.modules, {"winreg": winreg}), \
                mock.patch.object(
    ...
    def test_failed_startup_replacement_keeps_the_predecessor(self):
        values = {"Dikte": "old command"}
        winreg = self.winreg(values, OSError("access denied"))
        with mock.patch.object(reisulkuttab.sys, "platform", "win32"), \
                mock.patch.dict(sys.modules, {"winreg": winreg}), \
                mock.patch.object(reisulkuttab, "_launch_command",
                                  return_value="new command"), \
                mock.patch("builtins.print"):
            reisulkuttab._migrate_launch_at_login()

        self.assertEqual(winreg.values, {"Dikte": "old command"})
...
class LocalModels(AppTest):
    ...
    def test_nothing_is_fetched_for_a_window_nobody_opened(self):
        # AppTest closes the network, so a request would fail the test. The
    ...
    def test_only_the_chosen_cleaner_is_on_screen(self):
        window = self.window(
            self.config(cleanup_provider="openrouter"))
