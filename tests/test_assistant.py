...
import assistant
from tests.support import AppTest, fake_urlopen, only_these_tools
...
class AskClaude(AppTest):
    ...
    def test_the_prompt_goes_in_as_one_argument(self):
        ...
        self.assertEqual([os.path.basename(cmd[0]), *cmd[1:3]],
                         ["claude", "-p", "book it"])
...
class AskCodex(AppTest):
    ...
    def test_a_conversation_is_resumed(self):
        ...
        self.assertEqual([os.path.basename(cmd[0]), *cmd[1:4]],
                         ["codex", "exec", "resume", "t-1"])

    def test_a_fresh_conversation(self):
        _, cmd, _ = self.run_ask()
        self.assertEqual([os.path.basename(cmd[0]), cmd[1]], ["codex", "exec"])
