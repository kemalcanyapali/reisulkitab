#!/usr/bin/env bash
# Reisülküttab updater: pull, put the launchers back, restart what was running.
set -euo pipefail
...
# The combination stored in the settings, which is where Reisülküttab itself reads it
# from and the one place that is the same on KDE and on GNOME.
setting() {
  [[ -n "$PY" ]] || return 0
  "$PY" "$DIR/reisulkuttab.py" config get "$1" 2>/dev/null || true
}

echo
echo "Updating Reisülküttab"
echo "──────────────"
...
# writing them.
was_running=0
if pgrep -u "$USER_NAME" -f '(^|/)(dikte|reisulkuttab)\.py( |$)' \
    >/dev/null 2>&1; then
  was_running=1
fi
