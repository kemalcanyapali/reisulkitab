...
# In the order they answer in. Local and subscription-backed CLIs need no API key.
CLEANUP_PROVIDERS = [
...
class SettingsWindow(QDialog):
    ...
    def _general_tab(self):
        ...
        self.ui_language.setToolTip(
            t("Restart Reisülküttab for the language change to reach every window.")
        )
    ...
    def _api_tab(self):
        ...
        self.local_preload = QCheckBox(t("Load the model when Reisülküttab starts"))
        ...
        self.cleanup_provider.setToolTip(t(
            "llama.cpp runs fully offline. Antigravity and Claude Code reuse "
            "their existing CLI login without a second API key. OpenRouter uses "
        ...
        self.local_llm_preload = QCheckBox(t("Load the model when Reisülküttab starts"))
    ...
    def _meeting_tab(self):
        ...
        for label, value in CLEANUP_PROVIDERS:
            self.meeting_provider.addItem(t(label), value)
    ...
    def _shortcut_tab(self):
        ...
        elif hotkey.desktop_name() == "Windows":
            explanation = t(
                "Reisülküttab listens for these combinations while it is running. "
                "Nothing is installed, and the shortcut keys are not typed into "
                "the focused application."
            )
        else:
            explanation = t(
                "Reisülküttab asks macOS for these combinations itself, while it is "
    ...
    def _install_buttons(install_handler, remove_handler):
        ...
        macOS has not: Reisülküttab asks for the combination itself while it runs, so
    ...
    def _save(self):
        ...
        conf["cleanup_provider"] = self.cleanup_provider.currentData() or "local"
        ...
        for which, (box, _status, _missing) in self._shortcut_rows.items():
            ...
            fallback = (cfg.DEFAULTS[spec.setting]
                        if hotkey.desktop_name() == "Windows" and spec.fallback
                        else spec.fallback)
            conf[spec.setting] = box.currentText().strip() or fallback
        ...
        try:
            ...
            print(f"reisulkuttab: could not trim the history ({exc})")
        self._load_history()  # the trim may just have dropped rows from the list
        self.applied.emit()
        QMessageBox.information(self, t("Reisülküttab Settings"), t("Saved successfully."))
    ...
    def _test_openrouter(self):
        key, base = self._typed_key("openrouter")
        self._test_key(
            "openrouter", lambda: api.openrouter_key_status(key, base))
    ...
    def _install_shortcut(self, which):
        ...
        combo = box.currentText().strip() or fallback
        if not combo:
    ...
    def _refresh_shortcut_status(self, which):
        box, status, missing = self._shortcut_rows[which]
        if hotkey.desktop_name() == "Windows":
            status.setText(
                t("Active while Reisülküttab is running.") if box.currentText().strip()
                else missing
            )
            return
