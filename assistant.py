...
def resolved_executable(binary):
    """An executable path that cannot be replaced from the current directory."""
    found = shutil.which(binary)
    if not found:
        return ""
    path = pathlib.Path(found).resolve()
    if path.parent == pathlib.Path.cwd().resolve():
        return ""
    return str(path)


def cli_environment():
    """Process environment without Reisülküttab's transcription/API credentials."""
    environment = dict(os.environ)
    for key in cfg._ENV_KEYS:
