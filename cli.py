"""Reisülküttab from a terminal, with everything the windows can do.
...
# shortcut has always relied on: press the key on a fresh login and Reisülküttab comes
# up recording.
...
def _utf8_stdio():
    """Keep Windows pipes able to carry the same text as the GUI."""
    if os.name != "nt":
        return
    streams = ((sys.stdin, "strict"), (sys.stdout, "replace"),
               (sys.stderr, "replace"))
...
def fail(opts, message, code=1, **extra):
    if opts.json:
        ...
        print(f"{brand.SLUG}: {message}", file=sys.stderr)
    return code
...
def _not_running(opts):
    return fail(opts, f"{brand.NAME} is not running. Start it with: {brand.SLUG}",
...
SECRET_KEYS = cfg._SECRET_SETTINGS
...
def cmd_test_key(opts):
    ...
    for name, who in cfg.TRANSCRIBERS.items():
        ...
        try:
            if name == "openrouter":
                # A custom compatible endpoint has no OpenRouter credit record,
                # so its model list is the portable authentication check.
                message = api.openrouter_key_status(
                    conf.openrouter_key(), conf[who.url])
            else:
...
def cmd_status(opts):
    ...
    if reply is None:
        ...
        out(opts, {"ok": True, "running": False,
            ...
            "Reisülküttab is not running.")
        return NOT_RUNNING
    if reply.get("legacy"):
        return fail(opts, "the running instance is from before it could answer "
                          f"questions; reload it with: {brand.SLUG} restart",
                    1, running=True)
    lines = [
...
def cmd_doctor(opts):
    ...
    local_transcription = target.provider == "local"
    transcription_ready = (
        bool(ggml.program_path(ggml.WHISPER, conf["local_binary"]))
        and ggml.have_model(ggml.whisper_model_path(target.model))
        if local_transcription else bool(target.api_key)
    )
    checks = {
        "programs": programs,
    ...
    lines = [f"{'v' if path else 'x'} {name:14} {path or 'not on your PATH'}"
             for name, path in programs.items()]
    lines += [
        (f"{'v' if transcription_ready else 'x'} local whisper.cpp, "
         f"transcribing on {target.model}") if local_transcription else
        (f"{'v' if target.api_key else 'x'} {target.service} key, "
         f"transcribing on {target.model}"),
        (f"{'v' if conf.openrouter_key() else 'x'} OpenRouter key, cleaning up on "
         f"{conf['cleanup_model']}") if cleaner == "openrouter" else
        (f"{'v' if conf['local_llm_model'] else 'x'} local model, cleaning up on "
         f"{cleanup.model(conf) or 'not selected'}") if cleaner == "local" else
        (f"{'v' if programs.get(cleanup.executable(cleaner)) else 'x'} "
         f"{cleanup.executable(cleaner)}, cleaning up on {cleanup.model(conf)}"),
        f"{'v' if checks['running'] else '-'} application "
...
EPILOG = f"""\
examples:
  {brand.SLUG} record --seconds 8 --json
  {brand.SLUG} transcribe talk.mp4 --srt -o talk.srt
  {brand.SLUG} ask "put that in my calendar on Thursday at three"
  {brand.SLUG} config set cleanup_model google/gemini-3.5-flash
  {brand.SLUG} history show 1
...
def run(argv):
    ...
    # No verb at all opens the settings window.
