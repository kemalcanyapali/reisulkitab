...
TRANSCRIPT_MARKER = "<!-- reisulkuttab:transcript -->"
...
class MeetingPipeline(QObject):
    ...
    def _work(self, entry):
        ...
        workdir = None
        try:
            transcript = self._stored_transcript(doc_path)
            if not transcript:
                if not wav_path.exists():
                    raise api.ApiError(t("The recording is gone: {path}", path=wav_path))
                workdir = tempfile.mkdtemp(prefix="reisulkuttab-meeting-")
            ...
            model_name = (self.conf["meeting_model"].strip() or None
            ...
            used_model = model_name or (
                self.conf["local_llm_model"] if provider == "local" else provider)
    ...
    def _stored_transcript(self, doc_path):
...
def new_base():
    """A sortable minute stem, with a suffix only for a same-minute collision."""
    base = time.strftime("%Y-%m-%d-%H%M")
    existing = {row.get("base") for row in cfg.read_meetings()}
    if base not in existing and not any(path.exists() for path in cfg.meeting_paths(base)):
        return base
    index = 2
    while True:
        candidate = f"{base}-{index}"
        if (candidate not in existing
                and not any(path.exists() for path in cfg.meeting_paths(candidate))):
            return candidate
        index += 1
...
def new_entry(base, duration):
    ...
    try:
        stamp = time.strptime(base[:15], "%Y-%m-%d-%H%M")
        when = time.strftime("%Y-%m-%d %H:%M", stamp)
    except ValueError:
        # Read old Linux stems too, so existing meeting history survives the port.
        try:
            stamp = time.strptime(base[:15], "%Y%m%d-%H%M%S")
            when = time.strftime("%Y-%m-%d %H:%M", stamp)
        except ValueError:
