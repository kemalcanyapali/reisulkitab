#!/usr/bin/env python3
"""Reisülküttab: press Ctrl+Space, talk, press again to transcribe, clean up and paste.
...
`reisulkuttab.py --help`; the only argument handled here is --gui, which is how the
...
from PyQt6.QtCore import (QElapsedTimer, QLockFile, QProcess, QSocketNotifier,
                          QTimer)  # noqa: E402
...
class Reisulkuttab:
    def __init__(self, app, release_instance=None):
        self.app = app
        self._release_instance = release_instance or (lambda: None)
        ...
        # Before anything of ours is started: a server from a Reisülküttab that was
        ...
        self.recorder.interrupted.connect(self._on_recorder_interrupted)
        ...
        self.evdev.released.connect(self._on_hotkey_released)
    ...
    def _build_tray(self):
        ...
        self.menu = QMenu()
        self.enabled_action = QAction(t("Dictation enabled"), self.menu)
        self.enabled_action.setCheckable(True)
        self.enabled_action.setChecked(bool(self.conf["dictation_enabled"]))
        self.enabled_action.toggled.connect(self._set_dictation_enabled)
        ...
        if sys.platform == "win32":
            self.launch_action = QAction(t("Launch at login"), self.menu)
            self.launch_action.setCheckable(True)
            self.launch_action.setChecked(_launch_at_login())
            self.launch_action.toggled.connect(self._set_launch_at_login)
        ...
        self.tray.setToolTip(t("Reisülküttab: ready"))
    ...
    def _set_icon(self, name):
        ...
        if icon.isNull():
            icon = QIcon(str(brand.asset("reisulkuttab.png")))
    ...
    def _refresh_tray(self):
        labels = {
            IDLE: ("Start recording", "audio-input-microphone", "Reisülküttab: ready"),
            RECORDING: ("Stop and transcribe", "media-record", "Reisülküttab: recording"),
            BUSY: ("Working…", "view-refresh", "Reisülküttab: working"),
        }
        ...
        self.toggle_action.setEnabled(
            bool(self.conf["dictation_enabled"])
            and (self.state == RECORDING
                 or (self.state == IDLE and not self.recording))
        )
        ...
        if self.state == IDLE and self.ask_state != IDLE:
            if self.ask_state == RECORDING:
                icon, tip = "media-record", "Reisülküttab: recording for Claude"
            else:
                icon, tip = "view-refresh", "Reisülküttab: talking to Claude"
        ...
        if self.state == IDLE and self.ask_state == IDLE and self.meeting_state != M_IDLE:
            if self.meeting_state == M_RECORDING:
                icon, tip = "media-record", t("Reisülküttab: in a meeting")
            else:
                icon = "view-refresh"
                tip = self.meeting_message or t("Reisülküttab: writing the meeting up")
            self._set_icon(icon)
    ...
    def _set_dictation_enabled(self, enabled):
        self.conf["dictation_enabled"] = bool(enabled)
        self.conf.save()
    ...
    def _set_launch_at_login(self, enabled):
        try:
            _write_launch_at_login(bool(enabled))
        except OSError as exc:
            self.launch_action.blockSignals(True)
            self.launch_action.setChecked(_launch_at_login())
            self.launch_action.blockSignals(False)
            self.tray.showMessage(
                "Reisülküttab", t("Could not update launch at login: {error}", error=exc),
                QSystemTrayIcon.MessageIcon.Warning, 8000)

    def _on_evdev(self, name):
        if name == "toggle" and not self.conf["dictation_enabled"]:
            return
        timer = self.last_evdev.get(name)
        if timer is None:
    ...
    def _on_hotkey_released(self, name):
        if (name == "toggle" and self.conf["hotkey_mode"] == "hold"
                and self.state == RECORDING and self.recorder_owner == DICTATION):
            self.stop()
    ...
    def _dictation_request(self, cmd, request, reply):
        before = self.state
        requested_paste = request.get("paste") if "paste" in request else None
        if cmd == "toggle":
            self.toggle()
        elif cmd == "stop":
            self.stop()
        ...
        if requested_paste is not None and before != RECORDING and self.state == RECORDING:
            self.paste_override[DICTATION] = requested_paste
    ...
    def _ask_request(self, request, reply):
        ...
        requested_paste = request.get("paste") if "paste" in request else None
        self.toggle_ask()
        if requested_paste is not None and before != RECORDING and self.ask_state == RECORDING:
            self.paste_override[ASK] = requested_paste
    ...
    def start_meeting(self):
        ...
        if not self.meeting_recorder.start(
            str(wav_path),
    ...
    def _meeting_tick(self):
        ...
        if self.state == IDLE:
            self.tray.setToolTip(
                t("Reisülküttab: in a meeting ({time})", time=_clock(seconds))
            )
    ...
    def _on_meeting_finished(self, base, title):
        ...
        self.tray.showMessage(
            t("Reisülküttab: the meeting is written up"), f"{title}\n{doc_path}",
            QSystemTrayIcon.MessageIcon.Information, 10000,
        )
    ...
    def _on_meeting_failed(self, _base, error):
        ...
        self.tray.showMessage(
            t("Reisülküttab: the meeting could not be written up"),
    ...
    def _on_meeting_error(self, message):
        """The meeting recorder could not start or finish cleanly."""
        self.meeting_ticker.stop()
        if self.overlay.state == "meeting":
            self.overlay.dismiss()
        self._set_meeting_state(M_IDLE)
    ...
    def _on_finished(self, _raw, text, warning, paste_warning):
        details = "\n".join(x for x in (warning, paste_warning) if x)
        if warning:
            # The raw text was still pasted. Keep rejected keys visible instead
            # of making a failed cleanup look like working dictation.
            self.overlay.show_warning(
                t("Cleanup failed; raw text was used: {error}",
                  error=warning.splitlines()[0])
            )
            title = t("Reisülküttab: cleanup failed")
        elif paste_warning:
            self.overlay.show_warning(t(
                "Pasted, but clipboard restore failed: {error}",
                error=paste_warning.splitlines()[0],
            ))
            title = t("Reisülküttab: clipboard restore failed")
        else:
        ...
        if title:
            self.tray.showMessage(
                title, details, QSystemTrayIcon.MessageIcon.Warning, 10000,
            )
        self._set_state(IDLE)
        self._settle(DICTATION, {"ok": True, "text": text, "raw": _raw,
                                 "warning": details})

    def _on_ask_finished(self, _raw, text, warning, paste_warning):
        warning = "\n".join(x for x in (warning, paste_warning) if x)
        agent = assistant.display_name(self.conf)
        if warning:
            ...
            self.tray.showMessage(
                t("Reisülküttab: {name} could not do all of it", name=agent),
    ...
    def _on_recorder_interrupted(self, message):
        """Finish a usable partial capture, while telling the user it was cut."""
        self.tray.showMessage(
            "Reisülküttab", message, QSystemTrayIcon.MessageIcon.Warning, 10000)
        (self.stop_ask if self.recorder_owner == ASK else self.stop)()
    ...
    def _apply_local(self):
        ...
        Doing it while Reisülküttab starts rather than on the first dictation is the
        ...
        def warm():
            for server, name in wanted:
                try:
                    ...
                    print(f"reisulkuttab: {name}: {exc}", file=sys.stderr)
        ...
        if frozen and os.name == "nt":
            ...
            started, _pid = QProcess.startDetached(
                ...
                ["-NoProfile", "-WindowStyle", "Hidden",
                 "-Command", command])
            if not started:
                print("reisulkuttab: could not launch restart helper",
                      file=sys.stderr)
                return
            self.shutdown()
...
def _launch_command():
    if getattr(sys, "frozen", False):
        return subprocess.list2cmdline([sys.executable, "--gui"])
    executable = pathlib.Path(sys.executable)
    pythonw = executable.with_name("pythonw.exe")
    if pythonw.exists():
        executable = pythonw
    return subprocess.list2cmdline(
        [str(executable), str(ipc.script_path()), "--gui"])


def _launch_at_login():
    if sys.platform != "win32":
        return False
    import winreg
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, RUN_KEY) as key:
            value, _ = winreg.QueryValueEx(key, brand.NAME)
        return bool(value)
    except OSError:
        return False


def _write_launch_at_login(enabled):
    if sys.platform != "win32":
        return
    import winreg
    with winreg.CreateKey(winreg.HKEY_CURRENT_USER, RUN_KEY) as key:
        if enabled:
            winreg.SetValueEx(key, brand.NAME, 0, winreg.REG_SZ, _launch_command())
        else:
            try:
                winreg.DeleteValue(key, brand.NAME)
            except FileNotFoundError:
                pass


def _migrate_launch_at_login():
    """Move predecessor startup state and refresh renamed install paths."""
    if sys.platform != "win32":
        return
    import winreg
    try:
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER, RUN_KEY) as key:
            try:
                winreg.QueryValueEx(key, "Dikte")
                old_exists = True
            except FileNotFoundError:
                old_exists = False
            try:
                current, _ = winreg.QueryValueEx(key, brand.NAME)
            except FileNotFoundError:
                current = None
            if not old_exists and current is None:
                return
            expected = _launch_command()
            if current != expected:
                winreg.SetValueEx(
                    key, brand.NAME, 0, winreg.REG_SZ, expected)
                written, _ = winreg.QueryValueEx(key, brand.NAME)
                if written != expected:
                    raise OSError("startup replacement could not be verified")
            if old_exists:
                winreg.DeleteValue(key, "Dikte")
    except OSError as exc:
        print(f"{brand.SLUG}: could not migrate launch at login ({exc})",
              file=sys.stderr)
...
def _claim_server(server, lock):
    """Own the process lock, then replace only an unresponsive IPC endpoint."""
    if not lock.tryLock(0):
        return "running"
    probe = QLocalSocket()
    probe.connectToServer(SERVER_NAME)
    if probe.waitForConnected(ipc.CONNECT_MS):
...
def _take_request(buffer, final=False):
    if len(buffer) > MAX_IPC_REQUEST:
        raise OverflowError
    if b"\n" in buffer:
...
def install_signal_handlers(app):
    ...
    holding its model in memory. Abrupt exits are covered separately by the
    POSIX pid sweep or the Windows kill-on-close job object.
...
def run_app(args):
    ...
    if sys.platform == "win32":
        import ctypes
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(brand.APP_ID)
    _migrate_launch_at_login()
    app = QApplication(sys.argv)
    app.setApplicationName(brand.NAME)
    app.setDesktopFileName(brand.SLUG)
    app.setWindowIcon(QIcon(str(brand.asset("reisulkuttab.ico"))))
    app.setQuitOnLastWindowClosed(False)
    # Before Reisülküttab is built, because building it is what may start a server, and
    ...
    if not QSystemTrayIcon.isSystemTrayAvailable():
        print(f"{brand.SLUG}: no system tray found, running anyway")

    cfg.DATA_DIR.mkdir(parents=True, exist_ok=True)
    instance_lock = QLockFile(str(cfg.DATA_DIR / f"{brand.SLUG}.lock"))
    server = QLocalServer()
    ...
    ownership = _claim_server(server, instance_lock)
    if ownership != "listening":
        if ownership == "running" and command:
            ipc.send(command)
            return 0
        if ownership == "running":
            return 0
        print(f"{brand.SLUG}: could not open the IPC socket: {server.errorString()}")
        return 1
    ...
    reisulkuttab = Reisulkuttab(app, release_instance)
    ...
    def on_connection():
        ...
        def dispatch(payload):
            ...
            if handled:
                return
            handled = True
            reisulkuttab.handle(_parse_request(payload), reply)
        ...
        def disconnected():
            payload = _take_request(buffer, final=True)
            if payload is not None and not handled:
                dispatch(payload)  # legacy clients sent no newline

        conn.readyRead.connect(read)
        conn.disconnected.connect(disconnected)

    server.newConnection.connect(on_connection)
    app.aboutToQuit.connect(reisulkuttab.shutdown)
    ...
    if command == "settings" or not reisulkuttab.conf.transcribe_ready():
        reisulkuttab.open_settings()
    elif command == "toggle":
        QTimer.singleShot(0, reisulkuttab.toggle)
    elif command == "ask":
        QTimer.singleShot(0, reisulkuttab.toggle_ask)
    elif command == "meeting":
        QTimer.singleShot(0, reisulkuttab.toggle_meeting)
