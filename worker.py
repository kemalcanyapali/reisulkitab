...
_paste_shutdown = threading.Event()


def shutdown_pasting():
    """Reject new paste transactions and wait for an active restore to finish."""
    _paste_shutdown.set()
    with _paste_lock:
        pass
...
class Pipeline(QObject):
    ...
    finished = pyqtSignal(str, str, str, str)  # raw, final, work warning, paste warning
    ...
    def _work(self, wav_path, duration, rms_values, ask, paste_override=None):
        ...
        try:
            ...
            with _paste_lock:
                if _paste_shutdown.is_set():
                    raise assistant.Cancelled()
                previous = (paste.require_clipboard()
                            if conf["restore_clipboard"] and wants_paste else None)
                temporary_sequence = None
                primary_paste_error = None
                try:
                    paste.copy(text)
                    temporary_sequence = paste.clipboard_sequence()
                    if wants_paste:
                        self.stage.emit(t("Pasting…"))
                        paste.press(conf["paste_shortcut"])
                except paste.PasteError as exc:
                    primary_paste_error = exc
                finally:
                    if previous is not None:
                        ...
                        time.sleep(0.35)
                        try:
                            paste.copy_bytes(previous, temporary_sequence)
                        except paste.PasteError as exc:
                            paste_warning = str(exc)
                if primary_paste_error is not None:
                    if paste_warning:
                        raise paste.PasteError(
                            f"{primary_paste_error}\n{paste_warning}"
                        ) from primary_paste_error
                    raise primary_paste_error
