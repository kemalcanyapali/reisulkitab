...
program Reisülküttab then runs.

The lists are read rather than kept. A model catalogue written into the source
means a release of Reisülküttab for every new model, and a pinned whisper.cpp version
means one for every whisper.cpp release; both of those are somebody else's news,
not Reisülküttab's. Answers are cached for a few hours, and a cache that has gone stale
is still a better answer than none when the network is down.

Nothing here imports the rest of Reisülküttab apart from the string table: this module
...
CACHE_DIR = (
    pathlib.Path(os.environ.get("LOCALAPPDATA")
                 or pathlib.Path.home() / "AppData/Local") / brand.SLUG / "cache"
    if os.name == "nt"
    else pathlib.Path(os.environ.get("XDG_CACHE_HOME")
                      or os.path.expanduser("~/.cache")) / brand.SLUG
)
...
def repos(author="", search="", limit=40, refresh=False):
    ...
    enough one to keep the list current without Reisülküttab being updated.
    """
