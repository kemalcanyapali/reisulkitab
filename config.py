...
def _directories(platform=None, name=brand.NAME, slug=brand.SLUG):
    """(settings, data), using the target platform's native layout."""
    platform = platform or sys.platform
    posix = platform != "win32" and sys.platform == "win32"
    path_type = pathlib.PurePosixPath if posix else pathlib.Path
    home = path_type(_HOME.as_posix()) if posix else _HOME
    if platform == "darwin":
    ...
    if platform == "win32":
        ...
        return roaming / slug, local / slug
    config_root = path_type(os.environ.get("XDG_CONFIG_HOME") or home / ".config")
    data_root = path_type(os.environ.get("XDG_DATA_HOME") or home / ".local/share")
    return config_root / slug, data_root / slug


def _legacy_directories():
    """All layouts used by predecessor and transitional Windows releases."""
    layouts = [_directories(name="Dikte", slug="dikte")]
    if sys.platform == "win32":
        layouts.append(_directories(slug=brand.NAME))
    layouts.append((_xdg("XDG_CONFIG_HOME", "~/.config") / "dikte",
                    _xdg("XDG_DATA_HOME", "~/.local/share") / "dikte"))
    return tuple(dict.fromkeys(layouts))


def _predecessor_path(path):
    candidate = path.with_name(path.name + ".predecessor")
    number = 2
    while candidate.exists():
        candidate = path.with_name(f"{path.name}.predecessor-{number}")
        number += 1
    return candidate


def _merge_tree(source, destination):
    """Move a directory tree without overwriting anything already present."""
    destination.mkdir(parents=True, exist_ok=True)
    for item in list(source.iterdir()):
        target = destination / item.name
        if not target.exists():
            shutil.move(str(item), str(target))
        elif (not item.is_symlink() and not target.is_symlink()
              and item.is_dir() and target.is_dir()):
            _merge_tree(item, target)
        else:
            shutil.move(str(item), str(_predecessor_path(target)))
    source.rmdir()


def _migrate_legacy_directories():
    """Move existing Dikte profiles on the first renamed launch."""
    new_directories = _directories()
    migrations = [
        (old, new)
        for old_directories in _legacy_directories()
        for old, new in zip(old_directories, new_directories)
    ]
    if not any(old.exists() for old, _ in migrations):
        return
    with _locked(new_directories[0] / ".profile-migration"):
        seen = set()
        for old, new in migrations:
            if old in seen or not old.exists():
                continue
            seen.add(old)
            (old / "dikte.lock").unlink(missing_ok=True)
            _merge_tree(old, new)
        (new_directories[1] / "dikte.lock").unlink(missing_ok=True)
...
def _locked(path):
    """Serialize a file transaction across Reisülküttab threads and CLI processes."""
    lock_path = path.with_name(path.name + ".lock")
    lock_path.parent.mkdir(parents=True, exist_ok=True)
    with _file_lock, open(lock_path, "a+b") as lock:
        if os.name == "nt":
            import msvcrt
...
def _atomic_text(path, text, private=False):
    """Replace one text file without sharing a predictable temporary name."""
...
MEETINGS_DIR = (pathlib.Path.home() / "MeetingNotes"
                if sys.platform == "win32" else DATA_DIR / "meetings")
...
## Summary
Exactly five concise bullet points covering what was discussed and where it landed.
...
## Özet
Konuşulanları ve varılan noktayı kapsayan tam beş kısa madde.
...
ASSISTANT_PROMPT_EN = """This request reached you from Reisülküttab, a dictation tool.
...
ASSISTANT_PROMPT_TR = """Bu istek sana Reisülküttab adlı bir dikte uygulamasından geldi.
...
DEFAULTS = {
    ...
    "local_gpu": True,
    "local_preload": True,          # load the model while Reisülküttab starts, rather
                                    # than on the first dictation
    "local_binary": "",             # empty -> whichever copy ggml.py finds

    "cleanup_enabled": False,
    "cleanup_provider": "local",  # local | openrouter | claude | agy
    "cleanup_model": "google/gemini-3.5-flash-lite",
    "cleanup_claude_model": "haiku",
    "cleanup_agy_model": "",        # empty -> Antigravity CLI default
    "cleanup_reasoning": "",
    ...
    "meeting_provider": "agy",  # local | openrouter | claude | codex | agy
...
_ENV_SETTINGS = {
    ...
    "REISULKUTTAB_TRANSCRIBE_PROVIDER": "transcribe_provider",
    "REISULKUTTAB_CLEANUP_PROVIDER": "cleanup_provider",
    "REISULKUTTAB_MEETING_PROVIDER": "meeting_provider",
}
_ENV_KEYS = frozenset({
    "OPENAI_API_KEY", "OPENAI_BASE_URL",
    "GROQ_API_KEY", "GROQ_BASE_URL",
    "OPENROUTER_API_KEY", "OPENROUTER_BASE_URL",
    "REISULKUTTAB_TRANSCRIBE_PROVIDER", "REISULKUTTAB_CLEANUP_PROVIDER",
    "REISULKUTTAB_MEETING_PROVIDER",
})
_SECRET_SETTINGS = ("openai_api_key", "groq_api_key", "openrouter_api_key")
_CORNER_MIGRATION = {
...
def _env_paths():
    """Trusted local .env files; existing environment values always win."""
    roots = [CONFIG_DIR]
    if not getattr(sys, "frozen", False):
        roots.insert(0, pathlib.Path(__file__).resolve().parent)
    seen = set()
    for root in roots:
        path = root / ".env"
        if path not in seen:
...
def _load_env():
    """Read trusted KEY=value files without exporting secrets to child tools."""
    values = {key: os.environ[key] for key in _ENV_KEYS if key in os.environ}
...
def _dpapi(value, protect):
    """Protect or unprotect one UTF-8 value for the current Windows user."""
    ...
    source = Blob(len(raw), ctypes.cast(source_buffer,
    ...
    try:
        transformed = ctypes.string_at(result.data, result.size)
    finally:
        local_free = ctypes.windll.kernel32.LocalFree
        local_free.argtypes = [ctypes.c_void_p]
        local_free.restype = ctypes.c_void_p
        local_free(result.data)
    return (base64.b64encode(transformed).decode("ascii")
            if protect else transformed.decode("utf-8"))


def _decode_secrets(stored):
    for setting in _SECRET_SETTINGS:
        value = stored.get(setting)
        if isinstance(value, str) and value.startswith("dpapi:"):
            try:
                stored[setting] = _dpapi(value.removeprefix("dpapi:"), False)
            except (OSError, ValueError, UnicodeError) as exc:
                print(f"{brand.SLUG}: could not unlock {setting} ({exc})",
                      file=sys.stderr)
    return stored


def _encoded_settings(data):
    stored = dict(data)
    if os.name == "nt":
        for setting in _SECRET_SETTINGS:
            value = stored.get(setting)
            if isinstance(value, str) and value:
                stored[setting] = "dpapi:" + _dpapi(value, True)
    return stored
...
class Config:
    ...
    def load(self):
        try:
            ...
            if isinstance(stored, dict):
                self.data.update({k: v for k, v in _decode_secrets(stored).items()
                                  if k in DEFAULTS})
        except FileNotFoundError:
            pass
        ...
        for variable, setting in _ENV_SETTINGS.items():
            if self.env.get(variable, "").strip():
                self.data[setting] = self.env[variable].strip()
    ...
    def save(self):
        ...
        with _locked(CONFIG_FILE):
            try:
                ...
                stored = {}
            changes = (self.data if not stored
                       else {key: self.data[key] for key in self._dirty})
            stored.update(_encoded_settings(changes))
            text = json.dumps(stored, ensure_ascii=False, indent=2)
            _atomic_text(CONFIG_FILE, text, private=True)
        self._dirty.clear()
        i18n.set_language(self.data["ui_language"])
    ...
    def api_key(self, setting):
        """A stored key, or the private environment fallback loaded by Reisülküttab."""
        return self[setting].strip() or self.env.get(setting.upper(), "").strip()
    ...
    def uses_local_llm(self):
        """Whether enabled cleanup or meeting minutes use local llama.cpp."""
        return ((self["cleanup_enabled"] and self["cleanup_provider"] == "local")
                or self["meeting_provider"] == "local")
...
def append_history(entry, limit=None):
...
def _write_history(lines):
    ...
    _atomic_text(HISTORY_FILE, "".join(lines))
...
def delete_history(rows):
    """Remove entries matched on content, without racing a concurrent append."""
...
def delete_meetings(bases):
    ...
    with _locked(MEETINGS_FILE):
        _write_meetings(
            [row for row in read_meetings() if row["base"] not in doomed])
    for base in doomed:
