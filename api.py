...
def _stop_using(conn):
    ...
    so the reconnect is turned off first. A blocking WinSock read is cancelled
    by closing its SOCKET immediately; detach keeps Python from later closing a
    recycled handle with the same integer value.
    """
    conn.auto_open = 0
    sock = getattr(conn, "sock", None)
    if sock is not None and os.name == "nt":
        import ctypes

        handle = sock.detach()
        closesocket = ctypes.windll.ws2_32.closesocket
        closesocket.argtypes = [ctypes.c_size_t]
...
def _require_safe_url(url):
    """Reject credentials or recordings sent over cleartext off this machine."""
    try:
        parsed = urllib.parse.urlsplit(url)
        host = parsed.hostname
        parsed.port
    except ValueError as exc:
        raise ApiError(t("Invalid provider URL: {url}", url=url)) from exc
    if not host or parsed.username is not None or parsed.password is not None:
        raise ApiError(t("Invalid provider URL: {url}", url=url))
    if parsed.scheme == "https":
        return
    try:
        loopback = parsed.scheme == "http" and (
            host.lower() == "localhost" or ipaddress.ip_address(host).is_loopback)
    except ValueError:
        loopback = False
    if not loopback:
        raise ApiError(t("Provider URLs must use HTTPS: {url}", url=url))
...
def _opened(req, timeout, aborter):
    ...
    opener = urllib.request.build_opener(
        _SafeRedirect(), _TrackedHTTP(sockets), _TrackedHTTPS(sockets),
    )
...
def _multipart(fields, file_field, file_path):
    ...
    boundary = f"----{brand.SLUG}" + secrets.token_hex(16)
    out = bytearray()
    ...
    ctype = ("audio/x-wav" if filename.lower().endswith(".wav")
...
def openrouter_key_status(api_key, base_url=OPENROUTER_URL):
    """Check a key against OpenRouter or a configured compatible endpoint."""
    if not api_key:
        raise ApiError(t("{service} API key is empty. Add it in Settings.",
                         service="OpenRouter"))
    if base_url.rstrip("/") != OPENROUTER_URL:
        host = urllib.parse.urlsplit(base_url).hostname
        service = ("Gemini" if host == "generativelanguage.googleapis.com"
                   else "OpenAI-compatible provider")
        count = len(openai_models(api_key, base_url, service))
        return t("{count} models loaded.", count=count)
    try:
