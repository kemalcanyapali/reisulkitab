...
<p align="center">
  <img src="docs/settings-general.webp" width="820" alt="Reisülküttab ayarları, Genel sekmesi">
</p>
...
reisulkuttab                  # ilk açılışta ayarlar penceresi gelir
```

Fedora'da paket adları farklı, Fedora'nın kendi depolarındaki `ffmpeg-free`
yetiyor çünkü Reisülküttab video dosyasının yalnızca ses izini alıyor, `ydotool` da
...
`brew install ffmpeg`, `pip install PyQt6`, sonra `python reisulkuttab.py`. Toplantı
için BlackHole ya da Loopback gerekiyor, hoparlörden çıkanı kimse vermiyor.

`install.sh` `reisulkuttab` komutunu, menü girdisini, oturum açılışında otomatik
...
`OPENROUTER_API_KEY` kullanılır; anahtarlar `~/.config/reisulkuttab/config.json`
içinde, izinler 600, Mac'te ise `~/Library/Application Support/Reisülküttab` altında.
...
| Kaydı iptal et | `Ctrl+Alt+Space`, tepsi menüsü, ya da `reisulkuttab cancel` |
| Ajana sesle komut ver | Tepsi menüsü → *Claude'a sor*, ya da `reisulkuttab ask` |
| Toplantıyı başlat / bitir | Tepsi menüsü → *Toplantı kaydet*, ya da `reisulkuttab meeting` |
| Ayarlar | Tepsi menüsü → *Ayarlar*, ya da `reisulkuttab settings` |
| Güncelleme sonrası yeniden yükle | Tepsi menüsü → *Yeniden başlat*, ya da `reisulkuttab restart` |
| Çık | Tepsi menüsü → *Çık*, ya da `reisulkuttab quit` |

Ekranın köşesindeki gösterge kırmızı kayıt noktasını, canlı ses dalgasını ve
süreyi, ardından hangi aşamada olduğunu gösterir. Odak almaz. Reisülküttab çalışırken
`Ctrl+Space`'e tekrar basmak bir şey yapmaz, sıraya da girmez. Reisülküttab ile ajana
...
yazılımın tamamını çalıştırabilsin diye: `reisulkuttab record --seconds 8` söyleneni
geri verir, `reisulkuttab transcribe konusma.mp4 --srt` altyazıyı yazar, ayarlar,
geçmiş ve toplantılar da yanlarında durur. Hepsini `reisulkuttab --help` sayar, hepsi
...
- **Yanlış duyulan kelimeler düzeltilir.** Konuşma modelleri özel isimlerde sesçe
  ...
  ```
- **Başarısız temizleme sessizce geçmez.** Reisülküttab kaybolmasın diye ham transkript
  yine yapıştırılır ama gösterge kehribar rengine döner ve nedenini söyler,
  normal bir çalışma gibi görünmez.
- **Reisülküttab bunun yerine bir komut da olabilir.** Kendi kısayolu transkripti
...
- **Toplantılar** mikrofonla hoparlör çıkışından aynı anda kaydedilir; kimin ne
  ...
  `~/.local/share/reisulkuttab/meetings` altına ve Ayarlar → Tutanaklar sekmesine
...
```
reisulkuttab.py          giriş noktası, tepsi simgesi, durum makinesi
...
yerleştirmenin yolu yok, `reisulkuttab.py` bu yüzden `QT_QPA_PLATFORM=xcb` ayarlar.
